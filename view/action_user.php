...
<?php 
	...
	if(isset($_POST['insertion'])){
		...
		$insert = $db-> query("INSERT INTO `users`(`user_id`, `fullname`, `username`, `password` , `phone` , `email`  , `date` ,`note` , `photo`) VALUES ('1','$fullname','$username', '$password' , '$phone' , '$email'  , '$date' , '$note' , '$img')");
    ...
    if (isset($_POST['id']) AND isset($_POST['edit'])) {
		...
		$sql   = "UPDATE `users` SET `fullname`='$fullname',`photo`='$img' ,`username`='$username',`password`='$password',`phone`='$phone',`email`='$email',`date`='$date',`note`='$note' WHERE `id`= '$id' LIMIT 1";
		$query = $db->query($sql);
